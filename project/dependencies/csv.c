...
gen_u8 gen_csv_parse_delimiter( gen_CSV_Object* root, char* text, gen_AllocatorInfo allocator, gen_b32 has_header, char delim )
{
	...
	do
	{
		...
		row_item.type         = gen_EADT_TYPE_STRING;
#ifndef GEN_PARSER_DISABLE_ANALYSIS
		row_item.name_style = gen_EADT_NAME_STYLE_NO_QUOTES;
#endif
		...
		if ( *p == '"' )
		{
			...
			row_item.string = b;
#ifndef GEN_PARSER_DISABLE_ANALYSIS
			row_item.name_style = gen_EADT_NAME_STYLE_DOUBLE_QUOTE;
#endif
			do
			{
		...
		else if ( *p )
		{
			...
			if ( gen_str_len( num_p ) > 2 && num_p[ 0 ] == '0' && ( num_p[ 1 ] == 'x' || num_p[ 1 ] == 'X' ) )
			{
		...
		case gen_EADT_TYPE_STRING :
			{
				...
				{
					case gen_EADT_NAME_STYLE_DOUBLE_QUOTE :
						{
					...
					case gen_EADT_NAME_STYLE_NO_QUOTES :
						{
#endif
...
void gen__csv_write_header( gen_FileInfo* file, gen_CSV_Object* header )
{
	...
	temp.type           = gen_EADT_TYPE_STRING;
